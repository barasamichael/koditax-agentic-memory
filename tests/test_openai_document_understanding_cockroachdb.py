"""Live CockroachDB and real OpenAI regressions for Document AI understanding."""
...
def test_openai_understanding_executes_real_provider_outside_transactions(
    ...
    inspection_lease = _claim_work_item(
    ...
    with psycopg.connect(database_url) as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT result.provider_result_id, result.provider_response_id,
                       result.validated_result
                FROM document_ai_provider_results AS result
                WHERE result.tenant_id = %s
                  AND result.processing_operation_id = %s
                """,
                (tenant_id, understanding_lease.processing_operation_id),
            )
            row = cursor.fetchone()

    assert row is not None
    validated_result = row[2]
    assert isinstance(validated_result, dict)
    assert validated_result["structural_scope_ids"]


def test_canonical_assembly_persists_a_candidate_from_a_real_provider_result(
    cockroach_document_ai_database: str,
) -> None:
    _skip_if_live_dependencies_unavailable()
    database_url = cockroach_document_ai_database
    ...
    canonical_lease = _claim_work_item(
        database_url=database_url,
        work_kind="canonical_assembly",
    )
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                WHERE representation.tenant_id = %s
                  AND representation.processing_operation_id = %s
                ORDER BY element.ordinal ASC
                LIMIT 1
                """,
                (tenant_id, canonical_lease.processing_operation_id),
            )
            row = cursor.fetchone()

    assert row is not None
    assert str(row[0]) == "candidate"
    assert str(row[1]) == "none"
    assert isinstance(row[2], dict)
    assert row[2]["source_lineage"]["provider_result_id"]
    assert isinstance(row[3], dict)
    assert (
        row[3]["source_lineage"]["provider_result_id"]
        == row[2]["source_lineage"]["provider_result_id"]
    )
