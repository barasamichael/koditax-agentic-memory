"""Governed reports execution checks for orchestration runtime."""
...
def test_income_tax_reports_execution_returns_structured_and_synthesized_response() -> None:
    app = create_app(
        llm_response_generator=_build_generator(
            answer_text="The governed income tax report artifact has been generated successfully."
        )
    )
    client = TestClient(app)
    payload = _execute_payload_for_prompt(
        client,
        conversation_id="conv-reports-income-001",
        prompt_text="generate report for income tax audit trail.",
        user_id="user_reports_income_001",
        idempotency_key="idem-reports-income-001-v145",
    )

    first = client.post(
        "/v1/orchestration/prompt/execute",
        headers={"X-Correlation-ID": "corr-reports-income-001"},
        json=payload,
    )
    second = client.post(
        "/v1/orchestration/prompt/execute",
    ...
    body = first.json()
    assert body["selected_route"]["target_service"] == "reports"
...
def test_health_contribution_reports_execution_returns_governed_report_reference() -> None:
    app = create_app(
        llm_response_generator=_build_generator(
            answer_text=(
                "The governed health contribution report artifact is ready for downstream review."
            )
        )
    )
    client = TestClient(app)
    payload = _execute_payload_for_prompt(
        client,
        conversation_id="conv-reports-health-001",
        prompt_text="generate report for health contribution audit trail.",
        user_id="user_reports_health_001",
        idempotency_key="idem-reports-health-001-v145",
    )

    response = client.post(
        "/v1/orchestration/prompt/execute",
        headers={"X-Correlation-ID": "corr-reports-health-001"},
        json=payload,
    )
    ...
    assert body["response"]["answer_mode"] == "reports_execution"
    assert (
        body["adapter_response"]["result_payload"]["report_type"] == "health_contribution_summary"
    )
    assert body["response"]["assumptions"] == [
        "Report artifact details are limited to governed orchestration output."
    ]
