"""Implement governed non-resident employment income rule pack for 2021 historical window."""
...
SUPPORTED_BINDING_ID = "income_tax_non_resident_employment_v1_2021_01_01"
...
class SupportedNonResidentEmployment2021Input(BaseModel):
    """Represent the first supported non-resident-employment 2021 input lane."""
...
def execute_non_resident_employment_2021_rule_pack(
    prepared_input: PreparedExecutionInput,
    bound_rule: BoundRule,
) -> dict[str, object]:
    """Compute governed non-resident employment tax outcome for 2021 historical lane."""

    if bound_rule.binding_id != SUPPORTED_BINDING_ID:
        _raise_rule_input_error(
            reason="invalid_non_resident_employment_binding",
            message="Non-resident employment 2021 rule pack received an unexpected binding.",
            path="$.binding_id",
        )
    ...
    deductions_result = apply_supported_non_resident_employment_deductions_and_exemptions(
    ...
    reliefs_result = _apply_supported_non_resident_reliefs(payload.claims.relief_claims, gross_tax)
...
def _parse_supported_payload(
    prepared_input: PreparedExecutionInput,
) -> SupportedNonResidentEmployment2021Input:
    try:
        payload = SupportedNonResidentEmployment2021Input.model_validate(
            prepared_input.canonical_input_payload
        )
    except PydanticValidationError as error:
        _raise_rule_input_error(
            reason="unsupported_non_resident_employment_request_shape",
            message=(
                "Non-resident employment 2021 request does not match the supported governed shape."
            ),
            path="$.input_payload",
            details=str(error),
        )
...
def _validate_supported_version_context(
    ...
    if tax_year != 2021:
        _raise_rule_input_error(
            ...
            message="Non-resident employment 2021 rule pack supports tax year 2021 only.",
            path="$.tax_year",
        )
    if version_context.version_selection_basis != "specific_event_date":
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack requires "
    ...
    if not (
        ...
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack supports only the "
    ...
    if (
        ...
        _raise_rule_input_error(
            ...
            message=(
                "historical_version_id does not match the supported 2021 "
                "non-resident employment window."
            ),
...
def _validate_supported_taxpayer_context(
    ...
    if taxpayer_context.taxpayer_kind != "individual":
        _raise_rule_input_error(
            ...
            message="Non-resident employment 2021 rule pack supports individual taxpayers only.",
            path="$.input_payload.taxpayer_context.taxpayer_kind",
        )
    if taxpayer_context.resident_status_assertion != "non_resident":
        _raise_rule_input_error(
            reason="unsupported_resident_status",
            message="Non-resident employment 2021 rule pack supports non_resident taxpayers only.",
    ...
    if taxpayer_context.residence_reference_period_start not in (None, date(tax_year, 1, 1)):
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack supports whole-year "
                "non-resident scope only."
            ),
...
def _validate_supported_income_sections(income_sections: SupportedIncomeSections) -> None:
    if income_sections.business is not None:
        _raise_rule_input_error(
            ...
            message="Non-resident employment 2021 rule pack does not support business income.",
            path="$.input_payload.income_sections.business",
        )
    if income_sections.investment is not None:
        _raise_rule_input_error(
            reason="unsupported_income_domain",
            message="Non-resident employment 2021 rule pack does not support investment income.",
            path="$.input_payload.income_sections.investment",
        )
    if income_sections.rental is not None:
        _raise_rule_input_error(
            reason="unsupported_income_domain",
            message="Non-resident employment 2021 rule pack does not support rental income.",
    ...
    if income_sections.classification_pending_items:
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack does not support "
                "pending classification items."
            ),
    ...
    if not income_sections.employment.employment_items:
        _raise_rule_input_error(
            ...
            message="Non-resident employment 2021 rule pack requires employment items.",
    ...
    for index, item in enumerate(income_sections.employment.employment_items):
        if item.income_subtype != "cash_emolument":
            _raise_rule_input_error(
                ...
                message=(
                    "Non-resident employment 2021 rule pack supports cash_emolument items only."
                ),
                path=(
        ...
        if item.employer_reference_id is None:
            _raise_rule_input_error(
                reason="missing_employer_reference",
                message=(
                    "Non-resident employment 2021 rule pack requires employer_reference_id "
                    "to anchor section 5(1)(b) chargeability."
                ),
                path=(
                    "$.input_payload.income_sections.employment."
                    f"employment_items[{index}].employer_reference_id"
                ),
            )
        ...
        if amount < ZERO:
            _raise_rule_input_error(
                ...
                message="Non-resident employment amounts must be non-negative.",
                path=(
                    "$.input_payload.income_sections.employment."
...
def _validate_supported_payment_pathways(payment_pathways: SupportedPaymentPathways) -> None:
    if payment_pathways.withholding_events:
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack does not support withholding events "
                "in the first lane."
            ),
    ...
    if payment_pathways.installment_tax_events:
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack does not support installment-tax events "
                "in the first lane."
            ),
    ...
    if payment_pathways.advance_tax_events:
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack does not support advance-tax events "
                "in the first lane."
            ),
...
def _validate_supported_traceability(traceability_context: SupportedTraceabilityContext) -> None:
    if not traceability_context.source_record_ids:
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack requires "
                "source_record_ids for traceability."
            ),
    ...
    if traceability_context.completeness_assertion != "complete":
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack requires completeness_assertion=complete."
            ),
...
def _apply_supported_non_resident_reliefs(
    relief_claims: list[EmploymentReliefClaim],
    gross_tax: Decimal,
) -> dict[str, object]:
    if relief_claims:
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment 2021 rule pack does not support relief claims "
                "in the first lane."
            ),
