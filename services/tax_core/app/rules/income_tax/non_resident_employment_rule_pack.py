"""Implement the first governed non-resident employment income rule pack."""
...
SUPPORTED_BINDING_ID = "income_tax_non_resident_employment_v1_2023_07_01"
SOURCE_ANCHOR_IDS = ["ITA-2023-07-01-A", "AMEND-2023-FA"]
APPLIED_POLICY_IDS = [
    "ITC-POL-101",
    "ITC-POL-102",
    "ITC-POL-201",
...
class SupportedNonResidentEmploymentInput(BaseModel):
    """Represent the first supported non-resident-employment input lane."""
...
def execute_non_resident_employment_rule_pack(
    prepared_input: PreparedExecutionInput,
    bound_rule: BoundRule,
) -> dict[str, object]:
    """Compute governed non-resident employment tax outcome for first supported lane."""

    if bound_rule.binding_id != SUPPORTED_BINDING_ID:
        _raise_rule_input_error(
            reason="invalid_non_resident_employment_binding",
            message="Non-resident employment rule pack received an unexpected binding.",
            path="$.binding_id",
        )
    ...
    reliefs_result = apply_supported_non_resident_employment_reliefs(
    ...
    return {
        ...
        "domain_outcomes": {
            ...
            "prescribed_rate_resolution": _not_applicable_domain("NREM-20230701-008"),
...
def _parse_supported_payload(
    prepared_input: PreparedExecutionInput,
) -> SupportedNonResidentEmploymentInput:
    try:
        payload = SupportedNonResidentEmploymentInput.model_validate(
            prepared_input.canonical_input_payload
        )
    except PydanticValidationError as error:
        _raise_rule_input_error(
            reason="unsupported_non_resident_employment_request_shape",
            message="Non-resident employment request does not match the supported governed shape.",
            path="$.input_payload",
            details=str(error),
        )
...
def _validate_supported_version_context(
    ...
    if tax_year != 2023:
        _raise_rule_input_error(
            ...
            message="Non-resident employment rule pack supports tax year 2023 only.",
            path="$.tax_year",
        )
    if version_context.version_selection_basis != "specific_event_date":
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment rule pack requires "
    ...
    if not (
        ...
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment rule pack supports only the "
                "2023-07-01 to 2023-08-31 window."
            ),
    ...
    if (
        ...
        _raise_rule_input_error(
            ...
            message=(
                "historical_version_id does not match the supported non-resident employment window."
            ),
...
def _validate_supported_taxpayer_context(
    ...
    if taxpayer_context.taxpayer_kind != "individual":
        _raise_rule_input_error(
            ...
            message="Non-resident employment rule pack supports individual taxpayers only.",
    ...
    if taxpayer_context.resident_status_assertion != "non_resident":
        _raise_rule_input_error(
            ...
            message="Non-resident employment rule pack supports non_resident taxpayers only.",
    ...
    if taxpayer_context.residence_reference_period_start not in (None, date(tax_year, 1, 1)):
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment rule pack supports whole-year non-resident scope only."
            ),
...
def _validate_supported_income_sections(income_sections: SupportedIncomeSections) -> None:
    if income_sections.business is not None:
        _raise_rule_input_error(
            ...
            message="Non-resident employment rule pack does not support business income.",
            path="$.input_payload.income_sections.business",
        )
    if income_sections.investment is not None:
        _raise_rule_input_error(
            reason="unsupported_income_domain",
            message="Non-resident employment rule pack does not support investment income.",
            path="$.input_payload.income_sections.investment",
        )
    if income_sections.rental is not None:
        _raise_rule_input_error(
            reason="unsupported_income_domain",
            message="Non-resident employment rule pack does not support rental income.",
    ...
    if income_sections.classification_pending_items:
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment rule pack does not support pending classification items."
            ),
    ...
    if not income_sections.employment.employment_items:
        _raise_rule_input_error(
            ...
            message="Non-resident employment rule pack requires employment items.",
    ...
    for index, item in enumerate(income_sections.employment.employment_items):
        if item.income_subtype != "cash_emolument":
            _raise_rule_input_error(
                ...
                message="Non-resident employment rule pack supports cash_emolument items only.",
        ...
        if item.paye_withheld_kes is not None:
            _raise_rule_input_error(
                ...
                message=(
                    "Non-resident employment rule pack does not support payroll "
        ...
        if item.employer_reference_id is None:
            _raise_rule_input_error(
                ...
                message=(
                    "Non-resident employment rule pack requires employer_reference_id to "
                    "anchor section 5(1)(b) chargeability."
                ),
                path=f"$.input_payload.income_sections.employment.employment_items[{index}].employer_reference_id",
            )
...
def _validate_supported_traceability(
    ...
    if not traceability_context.source_record_ids:
        _raise_rule_input_error(
            ...
            message=(
                "Non-resident employment rule pack requires source_record_ids for traceability."
            ),
    ...
    if traceability_context.completeness_assertion != "complete":
        _raise_rule_input_error(
            ...
            message="Non-resident employment rule pack requires completeness_assertion=complete.",
