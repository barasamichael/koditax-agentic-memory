...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE audit_events
            ADD CONSTRAINT chk_audit_events_previous_event_hash_presence
            CHECK (
                fn_audit_events_previous_hash_presence_check(
                    user_id,
                    resource_type,
                    resource_id,
                    previous_event_hash
...
DROP TRIGGER IF EXISTS trg_audit_events_enforce_hash_chain ON audit_events;
CREATE TRIGGER trg_audit_events_enforce_hash_chain
    BEFORE INSERT ON audit_events
    FOR EACH ROW
    EXECUTE FUNCTION fn_audit_events_enforce_hash_chain();
